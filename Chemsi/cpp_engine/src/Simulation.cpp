...
std::vector<Species> Simulation::buildDefaultSpecies(ChemistryIndex& idxOut) {
...
knockdown_target_0_1_ =
    ...
    // Reactor step
    // --------------------
    double combustionHRR_W = 0.0;
    reactor_.step(dt, inhib_kgm3_, effectiveExternalCooling_W, combustionHRR_W);
    prof_add(ProfileStage::ReactorStep, 1);

    // Phase 2B/2C: apply knockdown to combustion heat release (HRR) deterministically.
    const double combustionHRR_raw_W = combustionHRR_W;
    combustionHRR_W *= (1.0 - clamp01(knockdown_0_1_));
